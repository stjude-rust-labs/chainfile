        ...
        for section_result in reader.sections() {
            ...
            for pair_result in section.stepthrough().map_err(Error::StepthroughError)? {
                ...
                let (start, end) = match pair.reference().strand() {
                    Strand::Positive => {
                        let start = match pair.reference().start().position().value() {
                            Value::Usize(a) => *a,
                            Value::LowerBound => {
                        ...
                        let end = match pair.reference().end().position().value() {
                            Value::Usize(b) => *b,
                            Value::LowerBound => {
                                unreachable!("lower bound not allowed on positive strand")
                            }
                    ...
                    Strand::Negative => {
                        let start = match pair.reference().end().position().value() {
                            Value::Usize(a) => a + 1,
                            Value::LowerBound => 0,
                        };

                        let end = match pair.reference().start().position().value() {
                            Value::Usize(b) => b + 1,
