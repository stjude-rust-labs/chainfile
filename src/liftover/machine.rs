    ...
    pub fn liftover(&self, interval: &Interval) -> Option<Vec<ContiguousIntervalPair>> {
        ...
        let (start, stop) = match interval.strand() {
            Strand::Positive => {
                ...
                let end =
                    interval.end().position().value().get().unwrap_or_else(|| {
...
mod tests {
    ...
    use genomics::core::position::zero::Position;
    ...
    pub fn test_valid_positive_strand_liftover() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(result.reference().contig().inner().as_str(), "seq0");
        ...
        assert_eq!(result.query().contig().inner().as_str(), "seq1");
