...
//! [`genomics::core::coordinate::zero::Coordinate`] from the reference
//! [`genomics::core::interval::zero::Interval`] to the query
//! [`genomics::core::interval::zero::Interval`] if the coordinate falls within
//! the [`Section`](crate::alignment::Section).
//!
...
//! lift over a [`genomics::core::interval::zero::Interval`] from the reference
//! genome to the query genome via [`liftover::Machine::liftover()`]. Perhaps
//! importantly (and different from most other liftover tools that the author is
//! aware of), this method provides the complete list of mapped contiguous
//! interval pairs that are encompassed by the provided interval rather than
//! providing an inexact mapping and/or lifting over a single position.
//!
//! Note that, while [`liftover::Machine::liftover()`] accepts a
//! [`genomics::core::interval::zero::Interval`], one can always lift over a
//! single position by constructing a 1-sized
//! [`genomics::core::interval::zero::Interval`] containing only the position in
//! question.
//!
...
//!
//! let interval = "seq0:+:3-4".parse::<genomics::core::interval::zero::Interval>()?;
